...
// General params
params.dir = 'result/bm'
params.out = 'benchmark'
params.help = false
...
process getEmpirical {
    ...
    ${params.model} -a $a -b $b -n $n -u $u -q $q -d $d -H $hk -v $y_var -c $y_cor -p $p_loc -s $p_sd --p_dist $p_dist -l $lambda -D $c_dist -r $r --C_mean $C_mean --C_var $C_var -S \$s -x 10 -k $chunk -m ${params.gen} -t ${params.t} -w ${params.which} -o empirical.txt
    """
}
...
process getProposed {
    ...
    script:
    """
    ${params.model} -a $a -b $b -n $n -u $u -q $q -d $d -H $hk -v $y_var -c $y_cor -p $p_loc -s $p_sd --p_dist $p_dist -l $lambda -D $c_dist -r $r --C_mean $C_mean --C_var $C_var -S ${params.sim} -x 10 -k 0 -m ${params.gen} -t ${params.t} -w ${params.which} -o ${params.out}
    """
}
