...
if (is.null(opt$geno) || is.null(opt$pheno) || is.null(opt$manta) ){
...
if (is.null(covariate_file)){
    res <- tryCatch( {manta(Y ~ ., data = data.frame(snp = X[, 1]), type = "I", subset = "snp")$aov.tab[1, 6]}, 
                     error = function(e){return(NA)} )
} else {
    res <- tryCatch( {manta(Y ~ ., data = data.frame(covariates, snp = X[, 1]), type = "I", subset = "snp")$aov.tab[1, 6]}, 
                     error = function(e){return(NA)} )
}
