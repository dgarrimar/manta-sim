...
option_list = list(
  ...
  make_option(c("--GTgen"), type="character", default="simPopStructure",
              help="genotype data generation: simPopStructure, simUnrelated, simRelated, simEmpirical [default %default]", metavar="character"),
  make_option(c("--GTdir"), type="character", default=NULL,
              help="input genotype directory with files generated by simulateGT.nf", metavar="character"),
...
if (is.null(opt$output) || is.null (opt$GTdir)){
...
if (PTgen == "matrixNorm"){
  ...
  ## 1. Effect of the SNP
   ...
   if (s != 0){
     # Load genotypes
     S <- fread(sprintf("%s/%s.gemma", GTdir, GTgen), data.table = FALSE, sep = ",")
  ...
  ## 2. Relatedness signal 
   ...
   Rg <- as.matrix(fread(sprintf("%s/%s.sXX.txt", GTdir, GTgen), data.table = FALSE, sep = "\t"))
