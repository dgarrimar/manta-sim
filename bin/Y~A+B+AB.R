...
option_list = list(
    ...
    make_option(c("--adonis"), type="numeric", default=0,
                ...
                metavar="numeric"),
    make_option(c("-Q", "--QuadForm"), type="character", default="farebrother-SS",
...
for (i in 1:S) {
    ...
    if (adonis != 0) {
        ...
        if (delta != 0) { # This should be commented if you want to generate B under H1 and eval the p-values
            ...
            setBlocks(perm) <- B
            ADONIS <- adonis(dist(Y) ~ A + B + A:B, permutations = perm, parallel = cores)$aov.tab[,6][1:3]
        } else {
	          ADONIS <- adonis(dist(Y) ~ A + B + A:B, permutations = adonis, parallel = cores)$aov.tab[,6][1:3]
        }
    }  
...
if (modelSim == "mvnorm") {
    ...
    params <- c(a, b, n, u, q, delta, hk, loc, pdist, sd, transf)
} else if (modelSim == "multinom") {
