...
option_list = list(
  ...
  make_option(c("--PTgen"), type="character", default="norm-0-1",
              help="Phenotype data generation error [default %default]", metavar="character"),
  make_option(c("--varE"), type="character", default="equal",
              help="Error variance: 'equal' or 'unequal' [default %default]", metavar="character"),
  make_option(c("--varG"), type="character", default="equal",
              help="Genetic variances: 'equal' or 'unequal' [default %default]", metavar="character"),
  make_option(c("--corE"), type="numeric", default=0,
              help="Error correlations [default %default]", metavar="numeric"),
  make_option(c("--corG"), type="numeric", default=0,
              help="Genetic correlations [default %default]", metavar="numeric"),
...
getCov <- function(q, v, c, tol = 1e-10){
  ...
  diag(corr) <- rep(1, q)
  sigma <- corr * tcrossprod(sqrt(vars))
