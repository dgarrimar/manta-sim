...
if("PCA" in grid.t) {
    ...
    */
    ...
    grid_ch.combine(geno_thin_ch, by:0).combine(kinship_ch, by: 0).spread(Channel.of("dummy")).set{gt2pt_ch}
...
process simulate_test {
    ...
    input:
    tuple id,n,q,PTgen,GTgen,s,hs2,hg2,alphaG,lambda,alphaH,file(bed),file(bim),file(fam),file(kinship),file(eigenval) from gt2pt_ch
    each v from Channel.fromList(1..params.p)
    // each v from Channel.fromList(1..3) // debugging
    each t from grid.t
    ...
    script:
    par = "$n|$q|$PTgen|$GTgen|$s|$hs2|$hg2|$alphaG|$lambda|$alphaH"
    par_gemma = "$par|GEMMA"
    par_mlm = "$par|MLM_$t"
    ...
    mlm.R -p pheno.txt -g geno -t $t -c $eigenval -n ${params.k} -k $kinship -v VC.txt --mlm mlm.assoc.txt --manova manova.assoc.txt
    """
}
...
process end {
   ...
   script:
   """
   sed -i "1 s/^/n\tq\tPTgen\tGTgen\ts\ths2\thg2\talphaG\tlambda\talphaH\tmethod\tmtc\ttie\\n/" $sim
   """
}
