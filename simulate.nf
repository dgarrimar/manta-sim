...
if (params.help) {
  ...
  log.info ' --t TRANSFORM               transformation of response variables: none, PCA (default: none)'
...
def grid = [:]
params.keySet().each{
  if(it in ['n','q','PTgen','GTgen','hs2','hg2','t','m','delta','hk','y_var','y_cor','p_loc','p_sd','p_dist','c_dist','lambda']){
    grid[it] = params[it]
  }
}
...
process simulate_test {
    ...
    script:
    par = "$n|$q|$PTgen|$GTgen|$d|$y_var|$y_cor|$c_dist|$hk|$p_loc"
    par_gemma = "$par|GEMMA"
    par_mlm = "$par|MLM_$t"
    ...
    for (( v=\$start; v<=(\$end); v++ )); do
       ...
       # Simulate phenotype
       simulatePT3.R -s \$v -n $n -q $q -m $PTgen --geno geno -o pheno.txt -d $d -v $y_var -c $y_cor -D $c_dist -H $hk -p $p_loc -S $p_sd --p_dist $p_dist -l $lambda
       ...
       mlm.R -p pheno.txt -g geno -t $t -c $eigenval -n ${params.k} --mlm mlm_\$v.assoc.txt --manova manova_\$v.assoc.txt $scale
    done
...
process end {
   ...
   script:
   """
   sed -i "1 s/^/n\tq\tPTgen\tGTgen\tdelta\ty_var\ty_cor\tc_dist\thk\tp_loc\tmethod\tmtc\ttie\\n/" $sim
   """
}
