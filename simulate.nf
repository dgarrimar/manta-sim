...
if (params.help) {
  ...
  log.info ' --scale SCALE               [WARNING] scale the response variables for MANTA (default: false)'
  ...
  log.info ' --delta DELTA               change in H1, 0 is H0 (default: 0)'
...
process simulate_test {
    ...
    for (( v=\$start; v<=(\$end); v++ )); do
        ...
        # Run MANTA/MANOVA
        if [[ $C == 'PCA' ]]; then
            manta.R -p pheno.txt -g geno -c $eigenval -k ${params.k} --manta manta_\$v.assoc.txt --manova manova_\$v.assoc.txt $scale 
        else 
            manta.R -p pheno.txt -g geno --manta manta_\$v.assoc.txt --manova manova_\$v.assoc.txt $scale 
        fi
